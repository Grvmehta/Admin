...
public class ListOfPackageCentralizedRate {
    ...
    public  void view() throws Exception{
        ...
        GenericMethods.driver.manage().timeouts().implicitlyWait(50, TimeUnit.SECONDS);
    	}
    	catch(Exception e)
    	{
    		throw e;
   ...
   //    System.out.println(data);
	...
	// new verification methods //
		...
		public  String verifyPackage_name(String value) throws Exception
	    {
			try
			{
	   ...
	   /*	 list2.removeAll(list1);
	   	 ...
	   	 for(int i=2;i<=count;i++)
	   	 {
	   		 ...
	   		 if(data.contains(attachPackages.c.trim()))
	   		 {
	   			 ...
	   			 Assert.assertEquals(src1.endsWith(value), true);
	   			 break;
	   		 }
		...
		public  void verifyAddActivationDate_linkCR(String value) throws Exception
		{
			try
			{
			ArrayList<String> arr=new ArrayList<String>();
			int item_count=GenericMethods.tr_count(trcount);
			System.out.println("Trcount before for"+item_count);
			for(int k=2;k<=item_count;k++)
			{
				String data=GenericMethods.driver.findElement(By.xpath("//table[@class='list_viewnew']//tr["+k+"]//td[3]")).getText();
				...
				if(data.contains(attachPackages.c.trim()))
				{
					...
					System.out.println("printing string:::"+str+"::::and value::"+value);
					Assert.assertEquals(str,value );
					System.out.println("2ns assertion pass and before break");
					break;
				}
			}
			Assert.assertEquals(arr.contains(attachPackages.c.trim()), true);
		...
		public  void verifyActivatedForever_textCR(String value1,String value2) throws Exception
		{
			Thread.sleep(7000);
			try
			{
			...
			for(int i=2;i<=tble_count;i++)
			{
				...
				if(data.contains(attachPackages.c.trim()))
				{
					...
					Assert.assertEquals(str, value1);
		   			String src1=GenericMethods.driver.findElement(By.xpath("//table[@class='list_viewnew']//tr["+i+"]//td[9]//img")).getAttribute("src");
		   			Assert.assertEquals(src1.endsWith(value2), true);
					break;
				}
			}
		...
		public  void afterPkgattach_status() throws InterruptedException
	    {
	    	int item_count=GenericMethods.tr_count(trcount);
			for(int k=2;k<=item_count;k++)
			{
	   ...
	   public  PackageDetailsCentralisedRate click_EditLinkCRPkg() throws InterruptedException
	   {
	   ...
	   public  void verify_GridPkgCR() throws InterruptedException
	   {
