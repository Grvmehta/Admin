...
public class UsersLevelList {
	...
	public  void verify_UserLevel(int iTestCaseRow) throws Exception
	{
		try
		{
		...
		for(int i=2;i<trcount;i++)
		{
           ...
           Assert.assertEquals(data, AddEditUserLevelPage.abc);
  		   ...
  		   Assert.assertEquals(src.endsWith(ExcelUtil.getCellData(iTestCaseRow, ExcelUtil.GetColumnIndex(Constant.Col_ExpectedResult2))),true);
	       break;
           }
		}
